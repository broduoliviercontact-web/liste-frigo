...
bool ListeFrigoTouch::poll(TouchEvent &event)
{
    ...
    if (!touched) {
        const bool should_emit_tap = tracking && !emitted;
        ...
        event.tab = detectNavTab(start_logical_x, start_logical_y);
        return true;
    }
...
{
    logical_x = LOGICAL_WIDTH - 1 - physical_y;
